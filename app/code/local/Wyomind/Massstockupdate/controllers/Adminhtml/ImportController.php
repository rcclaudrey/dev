...
class Wyomind_Massstockupdate_Adminhtml_ImportController extends Mage_Adminhtml_Controller_Action {
    ...
    public function loadfileAction() {
        ...
        if ($tmp_file == '') {
        ...
        } elseif (stripos($tmp_file, 'csv') < 1) {
            $rtn['status'] = 'error';
            $rtn['body'] = (Mage::helper('massstockupdate')->__('Wrong file type. "%s" must be a scv file.', $realPath));
            ...
            $offset = $sku_offset - 1;
            $l = 0;
            if ($this->getRequest()->getParam('enclosure') != "none") {
                while (false !== ($csvLine = $io->streamReadCsv($this->getRequest()->getParam('separator'), $this->getRequest()->getParam('enclosure'))) && $l < 1000) {
                ...
                while (false !== ($csvLine = $io->streamReadCsv($this->getRequest()->getParam('separator'))) && $l < 1000) {
                    $skus = array_splice($csvLine, $offset, 1);
                    array_unshift($csvLine, $skus[0]);
