...
Vikont_Pulliver_Helper_Data::inform("\nHelmet House Connector started...\n");

$commonHelper = Mage::helper('pulliver');
$moduleHelper = Mage::helper('pulliver/HelmetHouse');
...
try {
	...
	foreach($update as $itemNumber => $qtys) {
		if($sku = $skuHelper->getSkuByItemNumber('HH', $itemNumber)) {
			fputcsv($fileHandle, array($sku, $qtys[0], $qtys[1]));
			$lineCounter++;
		}
	}
