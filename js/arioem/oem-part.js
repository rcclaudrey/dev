...
function layoutTable(){
	...
	jQuery('span:contains("' + location.search.substr(1) + '")').closest('tr').children('td').css({
			background: '#FFB872',
			'font-weight': 'bold'
		});
/*
	var partTag = jQuery('span:contains("' + location.search.substr(1) + '")').closest('tr').children('.ariPLTag').text();
...
function createSidebar(){
	var myVar =  setInterval(function() {
		...
		if (jQuery('#ari_Assemblies_jl').is(':has(ul)') && jQuery( "#assembly-menu" ).is(':has(ul)') == false){
			...
			var modelTitle = queryParts[0] + '<br/>' + queryParts[1];
